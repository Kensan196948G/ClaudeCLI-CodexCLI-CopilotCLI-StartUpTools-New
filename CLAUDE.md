...
詳細は `claudeos/system/orchestrator.md` を参照。
...
グローバル設定 (`~/.claude/CLAUDE.md`) の Codex 運用ルールに従う。

- 通常レビュー: `/codex:review --base main --background` (PR 前必須)
- 対抗レビュー: 認証・DB・並列処理変更時に `/codex:adversarial-review`
- Debug: `/codex:rescue --background` (1仮説1rescue、最小修正、同一原因3回まで)
