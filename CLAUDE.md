...
### フェーズ遷移時の /compact 標準化（v8.1 追加）

各ループフェーズ切替時にコンテキストを圧縮し、Context Rot（文脈劣化）を防止する。
出典: Anthropic 公式ブログ "Using Claude Code: Session Management and 1M Context"
...
| 遷移タイミング | 推奨コマンド | 目的 |
|---|---|---|
| Monitor → Development | `/compact development focus on [対象Issue/PR番号]` | Issue分析結果を残し、状態確認の詳細を圧縮 |
| Development → Verify | `/compact verify focus on changed files and test plan` | 実装の試行錯誤を圧縮し、検証観点を明示 |
| Verify → Improvement | `/compact improvement focus on technical debt and refactor targets` | テスト結果を残し、修復ログを圧縮 |
| Improvement → Monitor | `/compact monitor focus on next backlog candidates` | 改善内容を残し、リファクタ詳細を圧縮 |

#### 適用基準

- **必須適用**: 1フェーズで以下のいずれかに該当した場合
  - 3,000行以上のツール出力が発生
  - rescue を 2 回以上実行
  - PR を 3 件以上処理
  - 同一フェーズが 60 分以上継続
- **任意適用**: 軽微なフェーズ（小Issue 1件のみ等）はスキップ可
- **禁止**: フェーズ未完了状態での `/compact` 実行（中途半端な圧縮を防ぐ）

#### コマンド使い分け

| 状況 | コマンド | 理由 |
|---|---|---|
| 同一プロジェクト内のフェーズ遷移 | `/compact [hint]` | 関連文脈を保持しつつ肥大化を防ぐ |
| 完全に新規Issueへ移行（前文脈不要） | `/clear` | Context Rot を完全リセット |
| 大量出力タスクの隔離 | サブエージェント委任 | 中間ノイズを親コンテキストに持ち込まない |

#### Context Rot 警告サイン

以下の兆候を検知したら即 `/compact` または `/clear` を実行：
- 同一指摘の繰り返し（Reviewer の発言が冗長化）
- フェーズ判定ミスの増加
- ツール選択の精度低下
