...
| Verify | QA → Reviewer → Security → DevOps（WebUI案件は + e2e-runner + security-reviewer） |
...
WebUI 案件では以下の3段階ゲートを適用する。
詳細な全250項目チェックリスト:
`Claude/templates/claudeos/docs/webui-full-verification-checklist.md`

| ゲート | タイミング | 内容 |
|---|---|---|
| Gate-1 | Verify フェーズ毎回 | lint / build / typecheck / unit test / API正常系 / security scan / E2E core / Playwright console error |
| Gate-2 | PR 作成直前 | 変更近傍テスト + 主要回帰 + CodeRabbit + secret scan + axe-core |
| Gate-3 | リリース前 Staging | 全250項目のうち必須・条件付きを全件実行（人間サインオフ必須） |
...
### WebUI案件の Verify フェーズ標準参加 Agent
