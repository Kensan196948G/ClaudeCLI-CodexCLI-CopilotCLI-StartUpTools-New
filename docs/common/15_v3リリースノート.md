...
| 項目 | 内容 |
|---|---|
| 送信先 | `CLAUDEOS_DEFAULT_TO` 環境変数で指定(未設定時 `CLAUDEOS_SMTP_USER`)|
| 送信元 | `CLAUDEOS_DEFAULT_FROM` 環境変数で指定(未設定時 `CLAUDEOS_SMTP_USER`)|
...
- cron は `~/.bashrc` を読まないため、利用者がセットアップ時に `[[ -f ~/.env-claudeos ]] && source ~/.env-claudeos` を `cron-launcher.sh` 冒頭に追記する必要があります(自動配置されません — テンプレ `Claude/templates/linux/cron-launcher.sh` には source 行は含まれず、`docs/common/16_HTMLメールレポート設定.md` のステップ 3 を参照して `sed -i '/^set -euo pipefail/a [[ -f ~/.env-claudeos ]] && source ~/.env-claudeos' ~/.claudeos/cron-launcher.sh` で 1 度だけ追記する)
