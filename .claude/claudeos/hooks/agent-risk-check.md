...
| 前段フックとの関係 | `safety-check` は 2026-04-15 に削除済み（Issue #121）。本フック単独で PreToolUse リスク判定を担う |
| 参照元 Issue | #104 |
...
- `safety-check` は 2026-04-15 の Frontier-Test (Issue #121) で削除済み。本フック単独で
  PreToolUse リスク判定を担当する（実装なし・agent-risk-check で機能上位互換）。
