...
> **🐛 v3.2.95 — statusLine をグローバル管理に一本化（メニュー12で全プロジェクト即時反映）**
> テンプレート `settings.json` から `statusLine` を削除し `~/.claude/settings.json` のみで管理。メニュー12選択で全プロジェクトに即時反映。詳細は [`CHANGELOG.md`](./CHANGELOG.md) を参照。
...
| 項目 | 状態 |
|------|------|
| バージョン | **v3.2.95** (statusLine グローバル管理一本化) — 旧: v3.2.94 |
