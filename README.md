...
> **🐛 v3.2.94 — statusline グローバル設定崩れを修正（SSH InitializeOnly）**
> `New-RemoteTemplateDeployScript` に `-InitializeOnly` スイッチを追加。SSH 起動時の `settings.json` 上書きによる statusLine 消失を防止。詳細は [`CHANGELOG.md`](./CHANGELOG.md) を参照。
...
| 項目 | 状態 |
|------|------|
| バージョン | **v3.2.94** (statusline グローバル設定崩れ修正) — 旧: v3.2.93 |
